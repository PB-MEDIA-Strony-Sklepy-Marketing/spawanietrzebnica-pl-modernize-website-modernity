...
/**
 * Resize image
 *  - provide crop to square size imageazyLoading
...
class ImageResizer
{
     ...
     * @param string  $destName     Dest name
     * @param integer $maxWidth     Max width
     * @param integer $maxHeight    Max height
     * @param string  $cropRatio    Crop Ratio
     ...
     *
     * @param string       $watermarkFile     Water mark file
     * @param string       $watermarkPosition Water mark position
     * @param string|array $watermarkPadding  Water mark padding
     *
     * @return void
     */
    public function addWatermark($watermarkFile, $watermarkPosition = '', $watermarkPadding = '')
    {
        ...
        if (is_array($watermarkPadding)) {
            ...
            $paddingArray = preg_split(',', $watermarkPadding);
     ...
     * @param boolean $output If true: echo image content otherwise return the content
     *
     * @return string
     */
    public function output($save = true, $output = true)
    {
        if (empty($this->size)) {
            return false;
        }
        ...
        if (((!$this->maxWidth && !$this->maxHeight) || ($this->maxWidth >= $width && $this->maxHeight >= $height)) && $this->watermarkFile === '') {
            if ($save) {
                ...
                //phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- fixme:: check output if escape $data
                echo $data;
            } else {
                return file_get_contents($this->sourceFolder . $this->sourceName);
            }
        } else {
            ...
            if ($xRatio > 1 && $yRatio > 1) {
                ...
                if ($xRatio * $height < $this->maxHeight) { // Resize the image based on width
                    $tnHeight = round($xRatio * $height);
                    $tnWidth = $this->maxWidth;
                } else {
                    $tnWidth = round($yRatio * $width);
            ...
            if (!$src || !imagealphablending($src, true)) {
                return false;
            }
            ...
            if ($save) {
                ...
                $temp_file = tempnam(sys_get_temp_dir(), 'wpsol_resize');
                $outputFunction($dst, $temp_file, $quality);
    ...
    /**
     * Find sharp - function from Ryan Rud (http://adryrun.com)
     *
     * @param string|integer $orig  Origin
     * @param string|integer $final Final
     *
     * @return mixed
     */
