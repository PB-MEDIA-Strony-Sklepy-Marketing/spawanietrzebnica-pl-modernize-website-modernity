...
class Admin
{
    ...
    public function loadAdminScripts()
    {
        ...
        if (strpos($current_screen->base, 'page_wpsol_speed_optimization') !== false) {
            wp_enqueue_style('wpsol-addon-admin-styles');

            // Material js
            wp_enqueue_script(
                'wpsol-addon-material-js',
                plugins_url('assets/js/material.min.js', dirname(__FILE__)),
                array('jquery'),
                WPSOL_ADDON_VERSION,
                true
            );
            wp_enqueue_style(
                ...
                plugins_url('/assets/css/material.deep_orange-amber.min.css', dirname(__FILE__))
            );
            // Tree Folder
            wp_enqueue_style('wpsol-addon-jaofiletree-css', plugins_url('/assets/css/jaofiletree.css', dirname(__FILE__)));
            wp_enqueue_script(
                'wpsol-addon-jaofiletree-js',
                plugins_url('assets/js/jaofiletree.js', dirname(__FILE__)),
                array('jquery'),
                WPSOL_ADDON_VERSION,
                true
            );
            ...
            wp_enqueue_script(
                ...
                plugins_url('assets/js/wpsol-addon-group-minify-js.js', dirname(__FILE__)),
                array('jquery'),
                WPSOL_ADDON_VERSION,
                true
            );
        ...
        if (strpos($current_screen->base, 'page_wpsol_speed_optimization') !== false) {
            wp_enqueue_script(
                ...
                plugins_url('assets/js/wpsol-addon-advanced-cdn.js', dirname(__FILE__)),
                array('jquery'),
                WPSOL_ADDON_VERSION,
                true
            );
            ...
            wp_localize_script('wpsol-addon-advanced-cdn-js', 'wpsolAddonParams', array('ajaxurl' => admin_url('admin-ajax.php'), 'wpsol_nonce' => wp_create_nonce('wpsol-addon-advanced-cdn-nonce')));
        }
    }
    ...
    public static function wpsolAddonPluginActivation()
    {
        ...
        $default_advanced = array(
            ...
            'iframe_video_lazy_loading' => 0,
            'remove_emojis' => 0,
            'disable_gravatar' => 0,
            'exclude_width_lazyload' => 0,
            'exclude_height_lazyload' => 0,
            'image_dimensions' => 0
        );
        ...
        // Create woocommerce speedup default
        $woo_speedup = get_option('wpsol_woocommerce_speedup');
        if (empty($woo_speedup)) {
            $woo_speedup = array();
        }
        $default_woo_speedup = array(
            'woo_disable_styles' => 0,
            'woo_disable_scripts' => 0,
            'woo_disable_fragments' => 0,
            'heartbeat_frequency' => '60',
            'woo_auto_cleanup' => 'disable'
        );
        $woo_speedup = array_merge($default_woo_speedup, $woo_speedup);
        update_option('wpsol_woocommerce_speedup', $woo_speedup);
    ...
    public static function wpsolAddonUpdateVersion()
    {
        ...
        if (version_compare($db_installed, '2.5.2', '<')) {
            $advanced['exclude_width_lazyload'] = 0;
            $advanced['exclude_height_lazyload'] = 0;
            update_option('wpsol_advanced_settings', $advanced);
        }

        if (version_compare($db_installed, '2.6.0', '<')) {
            $woo_speedup = array(
                'woo_disable_styles' => 0,
                'woo_disable_scripts' => 0,
                'woo_disable_fragments' => 0,
        ...
        if (version_compare($db_installed, '2.6.5', '<')) {
            $optimization['advanced_features']['defer_css'] = 0;
            $optimization['advanced_features']['defer_js'] = 0;
            update_option('wpsol_optimization_settings', $optimization);
        }

        if (version_compare($db_installed, '3.2.1', '<')) {
