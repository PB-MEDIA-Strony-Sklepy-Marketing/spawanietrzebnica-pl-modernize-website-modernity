...
/* Counter
/*-----------------------------------------------------------------------------------*/
add_shortcode('welderpro_counter', 'welderpro_counter_function');

	function welderpro_counter_function( $atts, $content=null ) {
		extract(shortcode_atts(array(
			'number' => '3541',
			'title'  => '',
			'color'  => '#666666'
...
// Icon Box Heading
add_shortcode('welderpro_iconbox', 'iconbox_func');
...
function iconbox_func($atts, $content = null)
{
	...
	extract(shortcode_atts(array(
		...
		if($iconimg == ''){
			...
			$symbol = "<img src='".esc_url($img_src[0])."' class='iconimg' alt='' />";
		}

		if($style == '1') {
			$out = '
			<div class="tl-spoiler pad0 contact iconbox '.esc_attr($class).' iconbox-style-' .esc_attr($style).' icon-color-'.esc_attr($iconcolor).' color-'.esc_attr($textcolor).'">
                        <div class="ok-icon" style="color:'.$titlecolor.'"> '.$symbol.' </div>
                         <div class="su-clearfix tl-spoiler-content" style="color:'.$contentcolor.'"><a href="'.$url.'"><h6 style="color:'.$titlecolor.'"> '. esc_html($title) .'</h6></a>'. do_shortcode($content) . '</div>
                 <a class="btns" style="background-color:'.$buttonbgcolor.'; color:'.$buttontcolor.';" href="'.$url.'">' .$buttontext. '</a></div>';
		}
		elseif($style == '2') {
			$out = '
		...
		elseif($style == '3') {
			$out = '<div class="col-xs-12 col-sm-12 col-md-12 feature choose-section ' .esc_attr($class). ' wpb_content_element iconbox-style-'.esc_attr($style).' icon-color-'.esc_attr($iconcolor).' color-'.esc_attr($textcolor).'">
						...
						<a href="'.$url.'"><h4 class="text-uppercase">'. esc_html($title) .'</h4></a>
...
function foliof_func($atts, $content = null){
	...
	<section id="project-version-one" class="projects-section projects-page-section">
			...
			<div class="normal-gallery project-box project-box2 col4 portfolio-scroller2" id="image-gallery-mix">
			<?php 
	        ...
	        ?>
				<div class="item project-posts single-project-item mix <?php echo esc_attr($cate_slug); ?>">
					<div class="img-wrap">
						<?php $original_image = wp_get_attachment_url(get_post_thumbnail_id());
						$thumb_w = '400';
						$thumb_h = '300';	  
                    ...
                    <img src="<?php echo esc_url($image);?>" alt=""></div>
						...
						<div class="hover-box">
							...
							<span><?php echo welderpro_excerpt(); ?></span>
						</div>
						</div>
							</div>
						</div>
	...
	<section id="projects-page-section" class="projects-page-section">
	        ...
	        ?>
				<div class="project-post project-post2 col-sm-6 col-xs-12 tank outside single-project-item mix <?php echo esc_attr($cate_slug); ?>">
					<div class="img-wrap">
						<?php $image = wp_get_attachment_url(get_post_thumbnail_id()); ?>
                    <img src="<?php echo esc_url($image);?>" alt="">
						<div class="content">
...
?>
	...
	<div id="mc_embed_signup">
		<form id="mc-embedded-subscribe-form" class="newsletters-1" action="<?php echo esc_url( home_url('/') ); ?>wp-content/plugins/newsletter/do/subscribe.php" onsubmit="return newsletter_check(this)">
			<input name="ne" class="newsletter-email" type="email" data-icon="icon-envelope2" placeholder="<?php _e('Enter your email address', 'welderpro'); ?>">
...
function ctabox_func($atts, $content = null)
{
	...
	<div class="callto-action text-center">
						<h3 class=""><span class="call-title-text"><span class="call-title-text"><?php echo htmlspecialchars_decode($title); ?></span></span><a href="<?php echo esc_html($link1); ?> " class="button-one"><?php echo htmlspecialchars_decode($btn1); ?></a></h3>
					</div>
    <?php
	...
	($query->have_posts()): $query->the_post(); ?>
					...
					<div class="img-box">
						...
						<div class="img-caption">
                            ...
                            <li><span><i class="fa fa-calendar"></i> &nbsp; <?php echo get_the_date('d'); ?>  / <?php echo get_the_date('M'); ?>  / <?php echo get_the_date('Y');?>  </span></li>
                            <li><span><i class="fa fa-comments-o"></i> &nbsp; <?php comments_number( esc_html__('0 comment', 'construct'), esc_html__('1 comment', 'construct'), esc_html__('% comments', 'construct') ); ?> </span></li>
                        </ul>
									<?php $content = get_the_content();
  $trimmed_content = wp_trim_words( $content, 15 ); ?>
  <p><?php echo $trimmed_content; ?></p>
									<a class="btn readmore" href="<?php the_permalink(); ?>"><?php _e('Read more', 'welderpro') ?></a>
								</div>
							</div>
							</article>
...
function testimonial_func($atts, $content = null)
{
	...
	<div class="testimonial-box testimonial-section testimonials">
	<?php
...
?>
          ...
          <div class="items bx-clone">
           		<?php $original_image = wp_get_attachment_url(get_post_thumbnail_id());
                    ...
                    <img src="<?php echo esc_url($image);?>" alt="">
           		<h2><a href="<?php the_permalink(); ?>" class=""><?php the_title(); ?></a></h2>
							<p><?php echo welderpro_excerpt(); ?></p>
							<a class="readmore" href="<?php the_permalink(); ?>"><?php _e('Read more', 'welderpro') ?></a>
          </div>
		<?php $i++; endwhile;?>
		<?php wp_reset_postdata(); ?>
	 	<?php endif; ?>
...
/* Imagebox 
/*-----------------------------------------------------------------------------------*/
if (!function_exists('welderpro_imagebox_function')) {
	function welderpro_imagebox_function( $atts, $content = null) {
		extract( shortcode_atts( array(
			'img' => '',
			'img_title' => '',
			'url' => '',
	...
	add_shortcode('welderpro_imagebox', 'welderpro_imagebox_function');
}
...
/* divider
/*-----------------------------------------------------------------------------------*/
if (!function_exists('welderpro_divider_function')) {
	function welderpro_divider_function( $atts, $content = null) {
		extract( shortcode_atts( array(
			'style' => '1',
			'icon' => '',
		...
		if($style == '8' && $icon != '') {
			$return2 = '<div class="welderpro-icon"><i class="fa '.esc_attr($icon).'"></i></div>';
		} else{
			$return2 = "";
		}
		  
		return '<div class="welderpro welderpro' .esc_attr($style). '" ' .$return. '>' .$return2. '</div>';  
	}
	add_shortcode('welderpro_welderpro', 'welderpro_divider_function');
}

/*-----------------------------------------------------------------------------------*/
/* New divider
/*-----------------------------------------------------------------------------------*/
if (!function_exists('welderpro_newdivider_function')) {
	function welderpro_newdivider_function( $atts, $content = null) {
		extract( shortcode_atts( array(
			'style' => 'solid',
			'line_color' => '#999999',
			'width' => '100%',
	...
	add_shortcode('welderpro_newdivider', 'welderpro_newdivider_function');
}

/*-----------------------------------------------------------------------------------*/
/* Headline
/*-----------------------------------------------------------------------------------*/
if (!function_exists('welderpro_headline_function')) {
	function welderpro_headline_function( $atts, $content = null) {
		extract( shortcode_atts( array(
			'type'   => 'h1',
			'margin' => '0 0 20px 0',
	...
	add_shortcode('welderpro_headline', 'welderpro_headline_function');
}
// Google Map
